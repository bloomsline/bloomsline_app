        ...
        <FadeIn style={{ paddingHorizontal: 22, paddingTop: 20 }}>
          <EdCard style={{ flexDirection: 'row', alignItems: 'center', gap: 16, marginBottom: 24 }}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={{ fontSize: 13, color: EDA.faint, marginTop: 1 }}>{tr.practitioner}</Text>
            </View>
          </EdCard>

          <MonoLabel color={EDA.faint} style={{ marginBottom: 6 }}>{tr.language}</MonoLabel>
          <Text style={{ fontSize: 13, color: EDA.inkSoft, marginBottom: 12, lineHeight: 18 }}>{tr.languageSub}</Text>
          ...
          <EdCard onPress={doSignOut} style={{ flexDirection: 'row', alignItems: 'center', gap: 14 }}>
            <View style={{ height: 38, width: 38, borderRadius: 19, backgroundColor: EDA.canvas, alignItems: 'center', justifyContent: 'center' }}>
              <LogOut size={17} color={EDA.faint} strokeWidth={2} />
            </View>
            <Text style={{ flex: 1, fontSize: 15, fontWeight: '600', color: EDA.ink }}>{tr.signOut}</Text>
          </EdCard>
        </FadeIn>
      </ScrollView>
    </View>
...
function ToggleOption({ Icon, label, selected, onPress }: { Icon: LucideIcon; label: string; selected: boolean; onPress: () => void }) {
  return (
    <TouchableOpacity
      ...
      activeOpacity={0.8}
      style={{
        flex: 1, backgroundColor: selected ? EDA.greenTint : EDA.card,
        borderWidth: 1.5, borderColor: selected ? EDA.green : EDA.line,
    ...
    >
      ...
      <Icon size={20} color={selected ? EDA.green : EDA.faint} strokeWidth={2} />
      <Text style={{ fontSize: 14, fontWeight: '700', color: selected ? EDA.greenDeep : EDA.inkSoft }}>{label}</Text>
    </TouchableOpacity>
  );
}
