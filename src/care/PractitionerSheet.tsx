  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <Pressable className="flex-1 justify-end bg-black/40" onPress={onClose}>
        <Pressable className="max-h-[85%] rounded-t-3xl" style={{ backgroundColor: EDA.card }} onPress={() => {}}>
          <View className="items-center pt-3">
            <View style={{ height: 4, width: 40, borderRadius: 2, backgroundColor: EDA.line }} />
          </View>
          <Pressable onPress={onClose} className="absolute right-4 top-4 z-10 h-9 w-9 items-center justify-center rounded-full" style={{ backgroundColor: EDA.greenTint }}>
            <X size={18} color={EDA.green} strokeWidth={2} />
          </Pressable>
          ...
          <ScrollView contentContainerStyle={{ paddingHorizontal: 24, paddingTop: 20, paddingBottom: 36 }} showsVerticalScrollIndicator={false}>
            <View style={{ alignItems: 'center' }}>
              {p?.photoUrl ? (
                ...
                <View style={{ width: 84, height: 84, borderRadius: 42, backgroundColor: EDA.green, alignItems: 'center', justifyContent: 'center', marginBottom: 14 }}>
                  <Text style={{ color: '#fff', fontWeight: '700', fontSize: 32 }}>{initial}</Text>
                </View>
              )}
              <Text style={{ fontSize: 22, fontWeight: '700', color: EDA.ink, textAlign: 'center' }}>{name ?? 'Your practitioner'}</Text>
              {subtitle ? <Text style={{ fontSize: 14, color: EDA.inkSoft, marginTop: 3, textAlign: 'center' }}>{subtitle}</Text> : null}
            </View>

            {facts.length > 0 && (
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: 10, marginTop: 20 }}>
                {facts.map((f) => (
                  <View key={f.label} style={{ backgroundColor: EDA.canvas, borderWidth: 1, borderColor: EDA.line, borderRadius: 14, paddingVertical: 10, paddingHorizontal: 14, minWidth: 96, alignItems: 'center' }}>
                    <MonoLabel color={EDA.faint} size={9.5}>{f.label}</MonoLabel>
                    <Text style={{ fontSize: 13.5, color: EDA.ink, fontWeight: '600', marginTop: 5, textAlign: 'center' }}>{f.value}</Text>
                  </View>
                ))}
              </View>
            )}
            ...
            {p?.bio ? (
              <View style={{ marginTop: 22 }}>
                <MonoLabel color={EDA.faint} style={{ marginBottom: 8 }}>{t.profile.about}</MonoLabel>
                <Text style={{ fontSize: 13.5, color: EDA.inkSoft, lineHeight: 21 }}>{p.bio}</Text>
              </View>
            ) : null}
          </ScrollView>
        </Pressable>
