...
- `.env.production` → `EXPO_PUBLIC_API_URL=https://www.bloomsline.com` + public OAuth client ids, dev flags emptied. Loaded automatically on the Scalingo build (the gitignored `.env` is absent there).
...
2. **Environment** → set **`NPM_CONFIG_PRODUCTION=false`** ⚠️ *(required — the build tools `expo`/`metro` are devDependencies; without this they won't install and the build fails. The buildpack prunes them from the runtime slug afterwards.)* `EXPO_PUBLIC_API_URL` is baked from `.env.production`; an env var of the same name set here **overrides it at build time**, which is how you point a review app at a different backend. Keep the two in agreement — a stale value in the file is invisible while the override exists, and becomes a broken build the moment it is removed.
3. **Code** → link GitHub `bloomsline/bloomsline_app`, auto-deploy the branch you want (`main` after merging, or `feat/signup-flow`).
4. **Deploy.** The buildpack runs `npm run build` (→ `dist/`) then `web` serves it. Live at `https://app.bloomsline.com` (custom domain; the default
...
- `MOBILE_WEB_ORIGINS = https://app.bloomsline.com` (CORS allow)
- `PATIENT_APP_URL = https://app.bloomsline.com` (invite link)

## OAuth (optional)
Email sign-in needs nothing extra. For Google/Microsoft on the web app, add
`https://app.bloomsline.com` as an authorized redirect URI in the
Google Cloud + Azure app registrations (as `http://localhost:8081` was in dev).

## Local production-build check
    EXPO_PUBLIC_API_URL=https://www.bloomsline.com npm run build
